...
<body>
<h1>EL을 이용해서 request에 세팅된 속성 얻어와서 출력하기</h1>
<ul>
 <li> [작성법] : \${K} </li>
 ...
 <li> List 객체 : ${strList} 
 <!--EL구문에서는 배열/리스트 관계 없이 요소 하나 얻어올 때 [index] 사용 -->
 <ul>
...
<li>DTO : ${book1}
 ...
 <!-- EL을 이용해서 필드 값을 얻어오려면 getter가 필수! -->
   ...
   <li>${book1.title}</li> <!-- book1.getTitle()와 동일 -->
   <li>${book1.writer}</li> 
   <li>${book1.price}</li>
...
<h1> empty 연산자 </h1>
  ...
  <ul>
	...
	<pre>
		...
		* 테스트 진행 시 쿼리 스트링을 이용 *
		...
		- 쿼리스트링 : 주소에 담긴 값을 표현하는 문자열
			...
			<h3>요청주소?K=V&K=V&K=V...</h3> 띄어쓰기 없이 무조건 이어쓰기
	...
	<h3>\${param.input2} : ${param.input2}</h3>
